...
def lambda_handler(event, context):
    ...
    # DEV B FEEDBACK: Only run the diff on the 'staging' branch to prevent duplicate work.
    if env != "staging":
    ...
    # 1. Load staging snapshot directly from the URI
    s3 = boto3.client("s3")
    ...
    # 2. Fetch the latest production snapshot
    try:
